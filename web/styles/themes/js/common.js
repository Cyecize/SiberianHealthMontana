...
// Listen for the event.
    ...
    cartNotiCont.addEventListener('triggerNoti', function () {
        //var data = {"products":0};
        //alert('Data Was Triggered!');
        $.ajax({
            url: "/check-for-prods",
        }).done(function(dataa) {
            var fixedJson = dataa.replace(/&quot;/g, '"');
            var jsonData = JSON.parse(fixedJson);

            if(jsonData["products"] > 0 ){
                $('.notify-bubble').show(150);
                $('.notify-bubble').html(jsonData['products']);
                //alert(jsonData['products'])
            }
        });
    });
...
function triigerCartUpdate() {
    cartNotiCont.dispatchEvent(customEvent);
}
